...
class Window implements WindowInterface
{
    ...
    private function getLeftAlignedContent(): array
    {
        ...
        {
            $contentLength = $this->visibleLength($content);
            $leftPaddingLength = $this->padding->getLeftPadding();
            $rightPaddingLength = $this->width - $contentLength - $leftPaddingLength - 2;
    ...
    private function getCenterAlignedContent(): array
    {
        ...
        foreach ($this->content as $content)
        {
            ...
            $totalPadding = max(0, $availableWidth - $contentLength);

            $leftPaddingLength = intdiv($totalPadding, 2);
    ...
    private function getRightAlignedContent(): array
    {
        ...
        foreach ($this->content as $content)
        {
            ...
            $leftPaddingLength = $this->width - $contentLength - $rightPaddingLength - 2;
    ...
    private function padRightVisible(string $text, int $width): string
    {
        $pad = max(0, $width - $this->visibleLength($text));
        return $text . str_repeat(' ', $pad);
    }

    private function visibleLength(string $text): int
    {
        return mb_strlen($this->stripAnsi($text));
    }
    ...
    private function padLeftVisible(string $text, int $width): string
    {
        $pad = max(0, $width - $this->visibleLength($text));
        return str_repeat(' ', $pad) . $text;
    }

    private function padBothVisible(string $text, int $width): string
    {
        $pad = max(0, $width - $this->visibleLength($text));
        $left = intdiv($pad, 2);
        $right = $pad - $left;
